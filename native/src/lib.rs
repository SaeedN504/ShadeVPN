...
fn build_config(profile: RealityProfile) -> Result<RealityConfig, String> {
    ...
    let sni = profile.sni.filter(|value| !value.is_empty()).ok_or_else(|| "Reality SNI is required".to_owned())?;
    ...
    Ok(RealityConfig {
        ...
        uuid,
        sni,
        public_key: "redacted".to_owned(),
        short_id: "redacted".to_owned(),
...
pub extern "system" fn Java_com_shadevpn_android_NativeBridge_nativeBuildLane(
    ...
    let result = read_profile(&mut env, profile_json).and_then(build_config).and_then(|config| {
    ...
    match result {
        ...
        Err(reason) => json_string(&mut env, &format!("{{\"lane\":\"invalid\",\"status\":\"rejected\",\"reason\":\"{}\"}}", reason.replace('"', "\\\""))),
    }
}
...
pub extern "system" fn Java_com_shadevpn_android_NativeBridge_nativeValidateProfile(
    ...
    match read_profile(&mut env, profile_json).and_then(build_config) {
        Ok(_) => json_string(&mut env, "{\"valid\":true,\"reason\":\"Reality profile parsed and VLESS framing validated\"}"),
        Err(reason) => json_string(&mut env, &format!("{{\"valid\":false,\"reason\":\"{}\"}}", reason.replace('"', "\\\""))),
    }
}
...
pub extern "system" fn Java_com_shadevpn_android_NativeBridge_nativeProbeControlPlane(
    ...
    let result = read_profile(&mut env, profile_json).and_then(build_config).and_then(|config| {
        ...
        let socket = address.to_socket_addrs().map_err(|_| "DNS resolution failed".to_owned())?.next().ok_or_else(|| "no resolved address".to_owned())?;
        TcpStream::connect_timeout(&socket, Duration::from_secs(8)).map(|_| "TCP endpoint reachable".to_owned()).map_err(|error| format!("TCP connect failed: {error}"))
    });
    match result {
        Ok(reason) => json_string(&mut env, &format!("{{\"reachable\":true,\"reason\":\"{}\"}}", reason.replace('"', "\\\""))),
